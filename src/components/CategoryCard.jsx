const CategoryCard = ({ data }) => {
  return (
    <section className="flex flex-col justify-center items-center w-full bg-sky-50 py-10 px-0">
      ...
      <div className="grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 w-full justify-items-center px-5 mx-auto">
        {data.map((category, index) => (
          <section
            key={index}
            className="relative overflow-hidden rounded-lg bg-gray-100 h-80 group m-10 group"
          >
            <img
              src={category.image}
              alt={category.title}
            ...
            <h3 className="absolute left-2 bottom-10 text-gray-200 text-xl font-semibold">
              {category.title}
            </h3>
